...
namespace ExcelDna.PackedResources
{
    internal class ExcelDnaPack
    {
        public static int Pack(string dnaPath, string xllOutputPathParam, bool compress, bool multithreading, bool overwrite, string usageInfo, List<string> filesToPublish, bool useManagedResourceResolver)
        {
            ...
            {
                if (filesToPublish == null)
                    ru.AddFile(File.ReadAllBytes(configPath), "__MAIN__", ResourceTypeName.CONFIG, false, multithreading);  // Name here must exactly match name in ExcelDnaLoad.cpp.
                else
            ...
            if (filesToPublish == null)
            {
                ru.AddFile(dnaContentForPacking, "__MAIN__", ResourceTypeName.DNA, false, multithreading); // Name here must exactly match name in DnaLibrary.Initialize.
                ru.EndUpdate();
            }
            else
            {
                ...
                foreach (ExternalLibrary ext in dna.ExternalLibraries)
                {
                    ...
                    if (ext.Pack)
                    {
                        ...
                        if (Path.GetExtension(path).Equals(".DNA", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (filesToPublish == null)
                            {
                                ru.AddFile(dnaContentForPacking, name, ResourceTypeName.DNA, compress, multithreading);
            ...
            foreach (Image image in dna.Images)
            {
                if (image.Pack)
                {
                    ...
                    if (filesToPublish == null)
                    {
                        ...
                        ru.AddFile(imageBytes, name, ResourceTypeName.IMAGE, compress, multithreading);
                ...
                foreach (SourceItem source in project.SourceItems)
                {
                    if (source.Pack && !string.IsNullOrEmpty(source.Path))
                    {
                        ...
                        if (filesToPublish == null)
                        {
                            ...
                            ru.AddFile(sourceBytes, name, ResourceTypeName.SOURCE, compress, multithreading);
