...
namespace ExcelDna.Integration
{
	...
	internal unsafe struct XlOper
	{
		...
		unsafe public struct XlReference
		{
			...
			// CONSIDER: Is this the right place - used in ObjectMarshaler and ObjectArrayMarshaler
			unsafe public static void SetExcelReference(XlOper* pOper, XlMultiRef* pMultiRef, ExcelReference r)
			{
				List<ExcelReference> refs = r.InnerReferences;

				pOper->xlType = XlType.XlTypeReference;
				pOper->refValue.SheetId = r.SheetId;

				pOper->refValue.pMultiRef = pMultiRef;
				pOper->refValue.pMultiRef->Count = (ushort)refs.Count;

				XlOper.XlRectangle* pRectangles = (XlOper.XlRectangle*)(&pOper->refValue.pMultiRef->Rectangles);
				pRectangles[0].RowFirst = (ushort)r.RowFirst;
				pRectangles[0].RowLast = (ushort)r.RowLast;
				pRectangles[0].ColumnFirst = (byte)r.ColumnFirst;
				pRectangles[0].ColumnLast  = (byte)r.ColumnLast;

				for (int i = 1; i < refs.Count; i++)
				{
					pRectangles[i].RowFirst = (ushort)refs[i].RowFirst;
					pRectangles[i].RowLast = (ushort)refs[i].RowLast;
					pRectangles[i].ColumnFirst = (byte)refs[i].ColumnFirst;
					pRectangles[i].ColumnLast  = (byte)refs[i].ColumnLast;
				}
			}
		}
	...
	internal class XlStringReturnMarshaler : ICustomMarshaler
	{
	...
	internal unsafe class XlBooleanReturnMarshaler : ICustomMarshaler
	{
		static ICustomMarshaler instance;
	...
	internal class XlDateTimeReturnMarshaler : ICustomMarshaler
	{
		static ICustomMarshaler instance;
	...
	internal class XlDoubleArrayMarshaler : ICustomMarshaler
	{
	...
	internal class XlObjectMarshaler : ICustomMarshaler
	{
		...
		unsafe public IntPtr MarshalManagedToNative(object ManagedObj)
		{
			...
			else if (ManagedObj is ExcelError)
			{
				XlOper* pOper = (XlOper*)pNative;
				pOper->errValue = (ExcelError)ManagedObj;
			...
			switch (type)
			{
				...
				case XlType.XlTypeReference:
					...
					else
					{
						...
						{
							r.AddReference(pAreas[i].RowFirst, pAreas[i].RowLast,
				...
				case XlType.XlTypeSReference:
					ExcelReference sref;
					sref = new ExcelReference(pOper->srefValue.Reference.RowFirst,
	...
	internal class XlObjectArrayMarshaler : ICustomMarshaler
	{
		...
		unsafe public IntPtr MarshalManagedToNative(object ManagedObj)
		{
			...
			for (int i = 0; i < rows * columns; i++)
			{
				...
				else if (obj is ExcelError)
				{
					pOper->errValue = (ExcelError)obj;
					pOper->xlType = XlType.XlTypeError;
				}
				else if (obj is bool)
				{
			...
			if (numReferenceOpers > 0)
			{
				...
				for (int i = 0; i < rows * columns && refOperIndex < numReferenceOpers; i++)
				{
					...
					if (pOper->xlType == XlType.XlTypeReference)
					{
						...
						XlOper.XlReference.SetExcelReference(pOper, (XlOper.XlMultiRef*)pCurrent, r);
